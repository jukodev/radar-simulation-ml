...
This script tests the LSTM model by sampling 100 predictions and visualizing
...
def generate_from_first(model, first_point, steps=200, device="cpu"):
    """
    Generate a trajectory from a first point using the LSTM model.
    
    :param model: Trained NextStepLSTM model
...
def create_position_heatmap(all_x: np.ndarray, all_y: np.ndarray, 
    ...
    """
    # Create 2D histogram
    H, x_edges, y_edges = np.histogram2d(all_x, all_y, bins=bins)
...
def create_altitude_heatmap(all_time: np.ndarray, all_fl: np.ndarray, 
    ...
    H, time_edges, fl_edges = np.histogram2d(all_time, all_fl, bins=bins)
    ...
    im = plt.imshow(
        H_log.T,
        origin="lower",
        extent=[time_edges[0], time_edges[-1], fl_edges[0], fl_edges[-1]],
        aspect="auto",
        cmap="inferno",
        interpolation="gaussian",
    )
    plt.colorbar(im, label="log(1 + count)")
    plt.title(f"LSTM Predicted Altitude over Time\n({NUM_PREDICTIONS} trajectories, {PREDICTION_STEPS} steps each)")
...
def main():
    ...
    all_y = []
    all_fl = []
    all_time = []  # Time values (packet index * 5)
    ...
    create_position_heatmap(
        all_x, all_y,
        output_path=OUTPUT_DIR / "lstm_position_heatmap.png"
    )
    
    print("\nCreating altitude heatmap...")
    create_altitude_heatmap(
        all_time, all_fl,
        output_path=OUTPUT_DIR / "lstm_altitude_heatmap.png"
    )
    ...
    create_position_heatmap(
        outer_x, outer_y,
        output_path=OUTPUT_DIR / "lstm_random_outer_position_heatmap.png"
    )
